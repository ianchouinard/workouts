...
import { EditGroup } from "../editGroup/editgroup.component";
...
export class GroupView {
    ...
    // stuff for workout selection
    public startTimes: any;
    public selectedToAdd: any;
    public otherWorkouts: any;
    ...
    public Index:any = this.navParams.get('index');

    editThisGroup () {
        this.navCtrl.push(EditGroup, { group: this.Group});
    }

    deleteThisGroup () {
        let confirm = this.alertCtrl.create({
            title: 'Delete Group',
            message: `Are you sure you want to delete the group ${this.Group.title}?`,
            buttons: [
                {
                    text: 'Cancel',
                    handler: () => {
                ...
                },
                {
                    text: 'OK',
                    handler: () => {
                        this.state.Groups.splice(this.Index, 1);
    ...
    private makeOtherWorkoutsList() {
        for (let workout of this.state.Workouts) {
            /*
                1. Need an array of starttimes for workouts in this group.
                2. In this loop, add workout to otherWorkouts if time doesn't match ^
            */
            if (!this.startTimes.includes(workout.startTime)) {
                this.otherWorkouts.push(workout);
            }
        }
    }
